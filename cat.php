...
<?php
  ...
  $url25 = "/cat.php?seat=25&adcar={$address_car}&pickdate={$pickup_date}&pickdrop={$pickup_drop}&renttime={$rent_time}&rentreturn={$rent_return}&adreturn={$ad_return}";
    // query bình thường
    $query      = "SELECT * FROM car WHERE address = '{$address_car}' AND active = 1 ";
    $query1    =  "SELECT * FROM car WHERE address = '{$address_car}' AND active = 1 " . $limit;
    if(isset($_GET['seat'])){
      $seat = $_GET['seat'];
      if($seat < 24){
        $query1 = "SELECT * FROM car WHERE address = '{$address_car}' AND active = 1 AND seat = {$seat} ".$limit;
      }else{
        $query1 = "SELECT * FROM car WHERE address = '{$address_car}' AND active  = 1 AND seat > {$seat} ".$limit;
      }
    }

    $url      = "/cat.php?adcar={$address_car}&pickdate={$pickup_date}&pickdrop={$pickup_drop}&renttime={$rent_time}&rentreturn={$rent_return}&adreturn={$ad_return}&desc=ORDER BY datecost DESC";
    $desc = '';
    $asc  = '';
    if(isset($_GET['desc'])){
      $desc = $_GET['desc'];
      ...
      if($desc != ""){
        $query1  = $query. $desc .' '. $limit;
        $url      = "/cat.php?adcar={$address_car}&pickdate={$pickup_date}&pickdrop={$pickup_drop}&renttime={$rent_time}&rentreturn={$rent_return}&adreturn={$ad_return}&asc=ORDER BY datecost ASC";
      }elseif($asc != ""){
        $query1  = $query .  $asc .' '. $limit;
        $url      = "/cat.php?adcar={$address_car}&pickdate={$pickup_date}&pickdrop={$pickup_drop}&renttime={$rent_time}&rentreturn={$rent_return}&adreturn={$ad_return}&desc=ORDER BY datecost DESC";
      }
...
?>
          ...
          <div class="pagin">
            <a class="page" style="color:white;border:1px solid;background-color:#f64512;padding: 3px 5px;margin:5px;" href="">1</a>
            <a class="page" style="color:white;border:1px solid;background-color:#f64512;padding: 3px 5px;margin:5px;" href="">2</a>
            <a class="page" style="color:white;border:1px solid;background-color:#f64512;padding: 3px 5px;margin:5px;" href="">3</a>
          </div>
        </section>
        <!-- We Work-->
      </main>
